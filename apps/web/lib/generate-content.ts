...
/**
 * Generates content for a tool without requiring Anthropic API.
 * @param url The URL of the website to scrape.
...
/**
 * Generates content for a tool with categories and alternatives.
 * @param url The URL of the website to scrape.
 * @returns The generated content.
 */
export const generateContentWithRelations = async (url) => {
  const scrapedData = await scrapeWebsiteData(url);
  ...
  const schema = contentSchema.extend({
    categories: z
      .array(z.string())
      .transform((a) =>
        a.map((name) => categories.find((cat) => cat.name === name)).filter(isTruthy)
      )
      .describe(
        `Assign the open source software product to the categories that it belongs to. Try to assign the tool to multiple categories, but not more than 3. If a tool does not belong to any category, return an empty array.`
      ),
    alternatives: z
      .array(z.string())
      .transform((a) =>
        a.map((name) => alternatives.find((alt) => alt.name === name)).filter(isTruthy)
      )
      .describe(
        `Assign the open source software product to the proprietary software products that it is similar to. Try to assign the tool to multiple alternatives. If a tool does not have an alternative, return an empty array.`
      ),
  });
